...
function renderSettingsView() {
  ...
  const automationExample = {
    "build-peer-review": {
      "templateIds": [42],
      "templateNameIncludes": ["build"],
      "attachStdout": true,
      "successComment": "Job completed successfully for {{ticketKey}}.\nAWX job: #{{job.id}}\nTemplate: {{templateName}}",
      "failureComment": "AWX job #{{job.id}} failed for {{ticketKey}}.",
      "childSuccessTransitions": ["Done", "Completed", "Complete"],
      "parent": {
        "enabled": true,
        "comment": "To Build Peer Review",
        "transitions": ["Build Peer Review"],
        "fallbackTransitions": ["In Progress", "Start Progress"],
        "retryTransitions": ["Build Peer Review"]
      }
    },
    "generic-close-child": {
      "templateIds": [99],
  ...
  if (state.automationProfilesJson == null) {
    state.automationProfilesJson = JSON.stringify(state.config.automationProfiles || {}, null, 2);
  }
  wrap.appendChild(settingsSection('AWX → Jira automation profiles', [
    mk('p', { style: { fontSize: '11px', color: '#5e6670', lineHeight: '1.5', marginTop: '-4px' } }, [
      'Optional JSON automation templates. Link a template to AWX jobs with templateIds or templateNameIncludes. If no profile matches, COREX uses only generic child-ticket behavior. Supported placeholders: {{ticketKey}}, {{parentKey}}, {{job.id}}, {{job.name}}, {{templateName}}.',
    ]),
    settingsTextarea('Profiles JSON', state.automationProfilesJson, (v) => { state.automationProfilesJson = v; }, {
      minHeight: '260px',
      placeholder: JSON.stringify(automationExample, null, 2),
    }),
  ]));
