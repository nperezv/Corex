...
ipcMain.handle('ticketLinks:set', async (event, { key, templateId, templateName, automationProfileId }) => {
  const cfg = loadConfig();
  cfg.ticketLinks = cfg.ticketLinks || {};
  cfg.ticketLinks[key] = { templateId, templateName, automationProfileId: automationProfileId || '', linkedAt: new Date().toISOString() };
  saveConfig(cfg);
  return cfg.ticketLinks;
});
